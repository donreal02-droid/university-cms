...
const TeacherSchedule = () => {
  ...
  const fetchSchedule = async () => {
    try {
      ...
      const response = await api.get('/schedule/teacher/my-schedule');
      ...
      // Get unique subjects
      const uniqueSubjects = new Set(scheduleData.map(cls => cls.subject?._id || cls.subject)).size;

      //  Get actual student count from teacher/students endpoint
      let totalStudents = 0;
      try {
        const studentsRes = await api.get('/teacher/students').catch(() => ({ data: { data: [] } }));
        ...
        totalStudents = scheduleData.length * 30;
      }
      ...
      setStats({
        ...
        upcomingToday,
        weeklyHours,
        differentSubjects: uniqueSubjects,
        totalStudents: totalStudents // 👈 NOW USING ACTUAL COUNT
      });
  ...
  const showClassDetails = (cls) => {
    ...
    const departmentName = cls.department?.name || cls.department || 'General';
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-4 border border-gray-200 dark:border-gray-700 max-w-sm">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 flex items-center gap-2">
            ...
            <span className="font-medium">Semester:</span> {cls.semester || 'N/A'}
          </p>
          {cls.subject?.credits && (
            <p className="text-sm text-gray-600 dark:text-gray-300 flex items-center gap-2">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">My Teaching Schedule</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {user?.name} • {user?.department?.name || 'Faculty'} • Teacher
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.differentSubjects}</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">Total Students</p> {/* Changed from "Est. Students" to "Total Students" */}
            </div>
          </div>
        </div>
      ...
      {/* Schedule Display - Grid or List View */}
      {viewMode === 'grid' ? (
        /* Grid View (Same as student but teacher-focused) */
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 divide-y dark:divide-gray-700">
              ...
              <div key={day} className="p-4">
                ...
                <div className="space-y-3">
                  {dayClasses.map((cls, idx) => {
                    const status = getClassStatus(cls);
                    const statusColors = {
                      ongoing: 'border-l-4 border-green-500',
                      upcoming: 'border-l-4 border-yellow-500',
                    ...
                    return (
                      <div
                        key={idx}
                        className={`bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 ${statusColors[status]} cursor-pointer hover:shadow-md transition-shadow`}
                        onClick={() => showClassDetails(cls)}
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          ...
                          <span className={`text-sm px-2 py-1 rounded-full ${
                            status === 'ongoing' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
          ...
          {schedule.length === 0 && (
            <div className="p-8 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-300">No classes scheduled</p>
              <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Your schedule will appear here once assigned</p>
            </div>
          )}
        </div>
      )}
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">{departmentName} • Sem {cls.semester || 'N/A'}</p>
                    </div>
                  </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-blue-800 dark:text-blue-400 mb-1">Teacher Schedule Tips</h3>
            <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1 list-disc list-inside">
              <li>Switch between Grid and List view using the button above</li>
              <li>Green cards show your ongoing classes</li>
              <li>Yellow cards are upcoming classes</li>
              <li>Click on any class to see department and semester details</li>
              <li>The red line shows the current time in grid view</li>
            </ul>
          </div>
        </div>
      </div>
